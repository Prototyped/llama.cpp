...
public:
    ...
    // normalized, roped), one f32 row per compact complete block, written via set_rows.
    // For a single sequence, a position-block watermark tracks incremental validity;
    // seq_rm clamps it and replay recomputes the affected range. Unified multi-sequence
    // streams recompute all complete blocks because compact row ownership can change.
    // Unused rows are masked by the -inf bias. Single-stream memories only.
