    ...
    for (int64_t s = 0; s < n_ns; ++s) {
        ...
        for (int64_t j = 0; j < n_kv; ++j) {
            ...
            if (have_dead && blk_of[j] < 0 && !cells.is_empty(j)) {
                ...
                // full block does. The graph only enables block top-k for a contiguous single
                // text prefix: at most r-1 unpooled cells, with distinct position slots.
                // Holes or multiple sequences must instead use the per-cell lookup below.
        ...
        if (dirty_cells != nullptr) {
            ...
            // A unified stream may contain several sequences whose position blocks overlap.
            // Cache rows follow the compact block table, not positions. In that uncommon case
            // recompute every complete block and discard all incremental watermarks.
            if (!one_seq) {
                pooled_w.clear();
                GGML_ASSERT(n_bid <= n_dirty_max);
                for (int64_t i = 0; i < n_dirty_max; ++i) {
                    const bool live = i < n_bid;
                    dst_d_rows[i] = live ? i : dustbin;
                    for (int64_t sec = 0; sec < 4; ++sec) {
                        dst_d_pos[sec*n_dirty_max + i] = live ? loc_blk_pos[sec*n_blocks + i] : 0;
                    }
                    for (int64_t j = 0; j < r; ++j) {
                        dst_d_cells[i*r + j] = live ? loc_blk_cells[i*r + j] : 0;
                    }
                }
            } else {
                ...
                // Position block -> compact block: incomplete blocks pool nothing and
                // must not overwrite the next complete block's compact row.
...
bool llama_memory_hybrid_idx_context::qsa_block_topk_safe(const llama_ubatch & ubatch) const {
    if (!mem || !mem->get_mem_idx() || !ubatch.seq_id || !ubatch.seq_id[0] ||
            !ubatch.pos || ubatch.is_pos_2d()) {
        return false;
    }
    ...
    // A contiguous text prefix has only its final partial block. With holes or
    // a shifted prefix, several incomplete blocks can collide at idx % ratio
    // in the table's single spare row, silently dropping live attention keys.
    return cells.seq_pos_min(seq) == 0 &&
           (int64_t) cells.seq_pos_max(seq) + 1 == cells.get_used();
}
...
uint32_t llama_memory_hybrid_idx_context::qsa_pooled_n_dirty_max(const llama_ubatch & ubatch, uint32_t ratio) const {
    ...
    int n_seq_present = 0;
    for (int sq = 0; sq < LLAMA_MAX_SEQ; ++sq) {
        if (cells.seq_pos_min(sq) >= 0 && ++n_seq_present > 1) {
            return std::max<uint32_t>(1, cells.get_used()/ratio);
        }
    }
    ...
    // mrope repeats one position across an image, and set_input_qsa then indexes the blocks by
    // cell rank: the complete blocks follow the cell count, which runs ahead of the positions by
    // the image's size. Size for the larger of the two; the dirty count only grows with it
    if (ubatch.is_pos_2d()) {
        n_complete = std::max<int64_t>(n_complete, mem->get_mem_idx()->get_cells(seq).get_used()/ratio);
    }
