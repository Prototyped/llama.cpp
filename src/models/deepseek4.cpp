...
llama_model_deepseek4::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (is_media) {
            ...
            // hash_tokens is t_inp_tokens behind the prefetch op: identical values, but ordering the
            // expert loads before layer 0. The media path does not take this branch, so it is unaffected.
