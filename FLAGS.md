...
| `--prompt-decode-max N` | 640 | With phase switching, uncached prompt tails up to N tokens run at the decode ubatch instead of a round trip. | 200 measured +6.4% wall time against 640. |
...
| `LLAMA_BENCH_FIXED_STEP=<1..32>` | off | llama-bench fixed-position probe: hold a prefix, time verify steps of this width, check rollback determinism. | Measurement harness. |
...
| `MOE_ROUTING_OUT` / `MOE_ROUTING_TAGS` | `moe-routing.bin` / 0xf | `llama-moe-routing`: record per-layer routing to a file for offline studies. | Tool. |
