...
| **Heads-as-rows sparse FA** (`GGML_METAL_FA_SPARSE_HR`, default on) | upstream's sparse path runs the vec kernel, one query row and one head per threadgroup, and cost -12% prefill at 4k. This kernel makes the 12 heads sharing a KV head the rows of a simdgroup-matrix tile, so each selected K/V row is loaded once per token; the backend keeps dense FA below 3x `top_k`. pp2048 at 8k / 16k / 32k / 64k: 253 / 247 / 232 / 204 tok/s vs dense 241 / 213 / 177 / 174 |
...
| **Expert remap without graph splits** (`GGML_METAL_HOST_OPS`, default on) | the expert-cache remap was a CPU op between two GPU splits at each of the 48 MoE layers, so the GPU drained and waited for the host every time; it now runs on a host thread while the command buffer waits on an event. Byte-identical, decode +9-16% |
| **Hyper-connection mixer mat-vecs** | the mixers' up-projection has K = 320 (10 Q8_0 blocks), which the stock kernel ran at ~40 GB/s: 8 rows per simdgroup with no threadgroup reduction takes it from 81 to 47 us at 4 tokens, and 8 loads in flight per thread take the 4-row F32 inject from 38 to 23 us. Bit-identical, decode +3-5% |

These are GPU work and GPU idle removed, not I/O: stream statistics are unchanged across the change.
