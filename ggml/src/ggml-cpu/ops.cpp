...
void ggml_compute_forward_flash_attn_union(const struct ggml_compute_params * params, struct ggml_tensor * dst) {
    ...
    const bool mask_all = ggml_get_op_params_i32(dst, 2) != 0;
    ...
    for (int64_t h = 0; h < NH; ++h) {
        ...
        for (int64_t t = 0; t < NQ; ++t) {
            ...
            for (size_t c = 0; c < cols.size(); ++c) {
                ...
                s *= scale;
                // the dense prefix carries the ordinary (causal) mask; union entries only with mask_all
                if (mask && (cols[c] < n_dense || mask_all)) {
