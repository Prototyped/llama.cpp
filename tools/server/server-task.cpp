...
std::string server_task_result_metrics::to_metrics() {
    ...
    if (has_moe) {
        add_items("counter", {
            ...
            { "moe_stream_preloads_ready_total",  "MoE streaming: wave experts resident from the previous preload", (double) moe.n_preload_ready },
...
bool server_prompt_cache::save_direct(server_prompt_cache_state & st, llama_context * ctx_tgt, llama_context * ctx_dft, int32_t id_slot) {
    ...
    slot_ckpt_save(tmp_main, packed.data(), packed.size(), n_tokens, st.prompt.checkpoints);
...
bool server_prompt_cache::load_direct(const server_prompt_cache_state & st, llama_context * ctx_tgt, llama_context * ctx_dft, int32_t id_slot) {
    ...
    if (ctx_dft != nullptr && st.size_drft > 0 && !load_one(ctx_dft, st.path_drft())) {
        return false;
    }
...
bool server_prompt_cache::load(server_prompt & prompt, const server_tokens & tokens_new, llama_context * ctx_tgt, llama_context * ctx_dft, int32_t id_slot,
    ...
    if (it_best != states.end()) {
        ...
        if (it_best->on_disk()) {
            ...
            if (!load_direct(*it_best, ctx_tgt, ctx_dft, id_slot)) {
...
static bool server_prompt_cache_read_entry(server_prompt_cache_state & st, const llama_context * ctx_tgt, bool has_mtmd) {
    ...
    slot_ckpt_load(st.path, packed.data(), packed.size()*sizeof(llama_token), st.prompt.tokens.size(), st.prompt.checkpoints);
